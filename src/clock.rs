...
pub fn run(ic: &mut IS31FL3731<i2c::I2c>) {
    ...
    loop {
        ...
        old_m_time = set_pixel(ic, coordinate(now.second() as f32 / 59.0, 2), old_m_time);
        old_h_time = set_pixel(ic, coordinate(now.minute() as f32 / 59.0, 3), old_h_time);
        old_d_time = set_pixel(ic, coordinate(now.hour() as f32 / 23.0, 4), old_d_time);
